...
class HrContract(models.Model):
    ...
    def el_calculation(self, payslip):
        leaves = self.env['hr.leave'].search([
            ...
            ('holiday_status_id.work_entry_type_id.code', 'in',['LEAVE140','LEAVE110','LEAVE120']),
            ('request_date_to', '>=', payslip.date_from),
            ('request_date_from', '<=', payslip.date_to),
        ])
        special_leaves = self.env['hr.leave'].search([
            ('employee_id', '=', self.employee_id.id),
            ('state', '=', 'validate'),  # ensure only approved leaves are counted
            ('request_unit_half', '=', False),  # ensure only full day leaves are counted
            ('holiday_status_id.work_entry_type_id.code', 'in', ['LEAVE160']),
            ('request_date_to', '>=', payslip.date_from),
            ('request_date_from', '<=', payslip.date_to),
        ])
        special_leaves_days = 0
        if special_leaves:
            special_leaves_days = sum(special_leaves.mapped('number_of_days'))
        total_leave_days = 0
        if leaves:
            total_leave_days = sum(leaves.mapped('number_of_days'))
            if total_leave_days > 3:
                ext_day = total_leave_days - 3
                ext_days = ext_day * 1.5
                total_leave_days  = total_leave_days + ext_days

        all_total_leave_days = special_leaves_days + total_leave_days
        ...
        # absent_days  =absent_data['absent_scheduled_days'] - len(valid_ph) - all_total_leave_days
        absent_days  =absent_data['absent_scheduled_days']


        if absent_days and  absent_days > 0:
            amount = (self.wage / no_of_days) * (absent_days * 1.5)
            return amount
        else:
            return 0
    ...
    @api.model
    def ld_calculation(self, inputs):
        obj_amount = sum(
            self.env['hr.payslip.input'].search([('payslip_id', '=', inputs.id), ('code', '=', 'LD')]).mapped(
                'amount'))
        return obj_amount

    @api.model
    def shd_calculation(self, inputs):
        obj_amount = sum(
            self.env['hr.payslip.input'].search([('payslip_id', '=', inputs.id), ('code', '=', 'SHD')]).mapped(
                'amount'))
        return obj_amount
    ...
    def hdl_calculation(self, payslip):
        leaves = self.env['hr.leave'].search([
            ...
            ('holiday_status_id.work_entry_type_id.code', 'in', ['LEAVE120','LEAVE160']),
            ('request_date_to', '>=', payslip.date_from),
            ('request_date_from', '<=', payslip.date_to),
        ])
        total_leave_days = 0
    ...
    def ph_days_calculation(self, payslip):
        ...
        if ph_obj:
            ...
            amount = (self.wage / no_of_days) * len(valid_ph)
            return amount
        else:
            return 0
    ...
    @api.model
    def sl_calculation(self, code, payslip):
        # Find all approved leaves that overlap the payslip period and match the type
        leaves = self.env['hr.leave'].search([
            ('employee_id', '=', self.employee_id.id),
            ('state', '=', 'validate'),  # ensure only approved leaves are counted
