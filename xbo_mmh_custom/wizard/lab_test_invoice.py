...
class medical_lab_test_invoice(models.TransientModel):
    _inherit = 'medical.lab.test.invoice'
    ...
    def create_lab_invoice(self):
        ...
        outsourced_lab_result_obj = self.env['outsourced.lab.request']
        ...
        for active_id in active_ids:
            if self._context['active_model'] == 'medical.patient.lab.test':
                ...
                invoice_vals = {
                    ...
                    'outsourced_lab_request': lab_req.outsourced_lab_request,
                }
            ...
            if self._context['active_model'] == 'outsourced.lab.request':
                lab_req = outsourced_lab_result_obj.browse(active_id)
                ...
                # 🔥 LOOP through test lines
                for line in lab_req.outsourced_lab_test_lines:

                    product = line.medical_test_type_id.service_product_id
                    ...
                    if not account_id:
                        raise UserError(
                            _('No income account for product: %s') % product.name
                        )
